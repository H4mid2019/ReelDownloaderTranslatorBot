...
class Transcriber:
    ...
    def _transcribe_with_google(self, audio_path: str) -> dict:
        ...
        if not _GOOGLE_AI_AVAILABLE:
            return {
                ...
                "google_translation_handled": False,
    ...
    def transcribe_audio(
        ...
        ext = (
            audio_file_path.rsplit(".", 1)[-1].lower() if "." in audio_file_path else ""
        )
        ...
        try:
            with open(audio_to_transcribe, "rb") as audio_file:
                ...
                response = self.client.audio.transcriptions.create(
                    file=(os.path.basename(audio_to_transcribe), audio_file), **params
                )
                ...
                # Get language name
                lang_name = (
                    self.LANGUAGE_NAMES.get(detected_lang, detected_lang)
                    if detected_lang
                    else "Unknown"
                )
                ...
                # Check if excluded
                is_excluded = (
                    detected_lang and detected_lang.lower() in self.EXCLUDED_LANGUAGES
                )
...
def transcribe_file(
    file_path: str, force_language: Optional[str] = None, use_local_ai: bool = False
) -> dict:
